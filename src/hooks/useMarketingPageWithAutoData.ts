...
import { useMarketingFormWithAutoData } from "./useMarketingFormWithAutoData";
...
export function useMarketingPageWithAutoData() {
  const { user } = useAuth();
  ...
  // Get form state with automatic data
  const {
    date,
    setDate,
    spendMAD,
  ...
  return {
    ...
    isLoadingAutoData,
    lastSyncAt,
    onRefreshAutoData: refetchAutoData,
  };
}
